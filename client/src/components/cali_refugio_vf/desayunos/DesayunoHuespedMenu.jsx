...
class DesayunoHuespedMenu extends Component {
    constructor(props){
        super(props);    
        this.state={
            estado: 'inicio',
...
toggleModalAceptar = () => {
    ...
    if(this.state.tipobebidadesayuno === '' || this.state.tipohuevosdesayuno === ''){
        ...
        let contPersonales = [JSON.parse(localStorage.getItem('Numero_Desayuno_Huesped'))]

        if(contPersonales[0] === null){
            //Guardamos en local Storage
            pedidoPizza = [{ 'key_id' : 1,
                        'tipo' : 'DESAYUNO HUESPED X' + this.state.cantidadDesayuno,
                        ...
                        'costo_desayuno_huesped' : this.state.cantidadDesayuno * this.state.costoPizzaPersonal,
                        'costo_adiciones_huesped' :this.state.cantidadDesayuno * costoadicion,
                        'id_pedido': 'Pedido_Desayuno_Huesped_0'}, this.state.ingredientes, this.state.ingredientesExtra];
            //insumosPizzaPersonal = this.state.ingredientes;
            localStorage.setItem('Pedido_Desayuno_Huesped_0', JSON.stringify(pedidoPizza))
            localStorage.setItem('Numero_Desayuno_Huesped', JSON.stringify({'Numero': 1}))
        }else{
            pedidoPizza = [{ 'key_id' : contPersonales[0].Numero + 1,
                        ...
                        'costo_desayuno_huesped' : this.state.cantidadDesayuno * this.state.costoPizzaPersonal,
                        'costo_adiciones_huesped' :this.state.cantidadDesayuno * costoadicion,
                        'id_pedido': `Pedido_Desayuno_Huesped_${contPersonales[0].Numero}`}, this.state.ingredientes, this.state.ingredientesExtra];
            localStorage.setItem(`Pedido_Desayuno_Huesped_${contPersonales[0].Numero}`, JSON.stringify(pedidoPizza))
            localStorage.setItem('Numero_Desayuno_Huesped', JSON.stringify({'Numero': contPersonales[0].Numero + 1}))
        }  
...
render(){
    const opcionPizza = () => {
        switch(this.state.estado) {
            ...
            <>
            <div>
                <p style={{textAlign: 'center'}}>DESAYUNO HUESPED</p>            
                ...
                <div className='pizzaforma'>                    
                    <div>
                        ...
                        <p>HUESPED {this.state.saborpizzaadicion}</p>
                        </div>
                    </div>
            ...
            <ModalHeader>Pedido DESAYUNO HUESPED</ModalHeader>
            <ModalBody>
...
export default DesayunoHuespedMenu